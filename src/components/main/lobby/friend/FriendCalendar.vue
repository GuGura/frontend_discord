                ...
                customButtons: {
                    today: {
                        ...
                        click: () => {
                            ...
                            RestApi.post("/event/listMonthlyFriend", {
                                date: this.getApi().getDate(),id:useFriendStore().user.id
                            }).then(({data}) => {
                                for (const i in data) {
                    ...
                    prev: { // this overrides the prev button
                        ...
                        click: () => {
                            this.getApi().prev();
                            this.getApi().removeAllEvents();
                            RestApi.post("/event/listMonthlyBtnFriend", {
                                date: this.getApi().getDate(),id:useFriendStore().user.id
                            }).then(({data}) => {
                                for (const i in data) {
                                    this.getApi().addEvent({
    ...
    methods: {
        ...
        handleEventClick(clickInfo) {
            if (confirm(`일정 '${clickInfo.event.title}'을 내 캘린더에 추가하시겠습니까?`)) {
                RestApi.post("/event/saveFriendEvent", {
